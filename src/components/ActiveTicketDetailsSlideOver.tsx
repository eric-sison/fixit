...
type ActiveTicketDetailsSlideOverProps = {
  open: boolean;
  setOpen: (open: boolean) => void;
  ticket: TicketSummary;
};

export const ActiveTicketDetailsSlideOver: FunctionComponent<ActiveTicketDetailsSlideOverProps> = ({
  open,
  setOpen,
  ticket,
}) => {
                    ...
                    <div className="flex h-full w-[40rem] flex-col bg-zinc-900 border-l border-l-zinc-800 ">
                      <header className="mb-2 border-b border-b-zinc-700 py-7 px-10">
                        <div className="flex items-center gap-4">
                          ...
                          <section>
                            <h3 className="text-3xl font-bold text-font-regular/90">{ticket.requestor.fullName}</h3>
                            <p className="font-semibold text-xl text-font-muted">{ticket.requestor.positionTitle}</p>
                          </section>
                        </div>
                      </header>
                      ...
                      <section className="flex flex-col gap-10 px-10">
                        <main className="mt-5">
                          <h2 className="text-2xl text-font-regular/80">{ticket.details}</h2>
                        ...
                        <section className="space-y-4">
                          <div className="border-l-4 pb-10 px-2 border-l-blue-700">
                            ...
                            <h3 className="text-lg inline font-bold text-font-regular">{ticket.requestor.fullName}</h3>
                            <p className="text-font-muted">{dayjs(ticket.createdAt).format("MMM DD, YYYY")}</p>
                          </div>
                          ...
                          <div className="border-l-4 pb-10 px-2 border-l-blue-700">
                            <h3 className="text-lg inline text-font-regular/90">Move to active by </h3>{" "}
                            <h3 className="text-lg inline font-bold text-font-regular">Eric Sison</h3>
                            <p className="text-font-muted">{dayjs(ticket.createdAt).format("MMM DD, YYYY")}</p>
                          </div>
                        </section>
